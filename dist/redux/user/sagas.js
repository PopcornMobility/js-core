...
function success(adminRoles) {
  return function* SUCCESS() {
    const user = yield select(getUser); // check if user has any other role besides "client"
...
export default function* rootSaga(config) {
  ...
  yield all([takeEvery(actions.LOGIN, login(url, extraParams)), takeEvery(actions.LOGOUT, LOGOUT), takeEvery(actions.SUCCESS, success(adminRoles))]);
}